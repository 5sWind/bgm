...
TxBrowser.prototype.submit = function () {
  ...
  try {
    ...
    if (this.txNumber.indexOf('0x') !== -1) {
      global.web3.bgm.getTransaction(this.txNumber, function (error, result) {
        self.update(error, result)
      })
    } else {
      global.web3.bgm.getTransactionFromBlock(this.blockNumber, this.txNumber, function (error, result) {
        self.update(error, result)
      })
    }
  } catch (e) {
...
TxBrowser.prototype.checkWeb3 = function (callback) {
  try {
    global.web3.bgm.getBlockNumber(function (error, block) {

'use strict'
var Bgmdebugger = require('../Bgmdebugger')
...
Debugger.prototype.debug = function (blockNumber, txNumber, tx, loadingCb) {
  ...
  try {
    if (txNumber.indexOf('0x') !== -1) {
      return web3.bgm.getTransaction(txNumber, function (_error, result) {
        let tx = result
        self.debugTx(tx, loadingCb)
      })
    }
    web3.bgm.getTransactionFromBlock(blockNumber, txNumber, function (_error, result) {
      let tx = result
      self.debugTx(tx, loadingCb)
    })
  } catch (e) {

...
class Mapping extends RefType {
  ...
  async decodeMappingsLocation (preimages, location, storageResolver) {
    var mapSlot = util.normalizeHex(bgmutil.bufferToHex(location.slot))
...
function getMappingLocation (key, position) {
  ...
  var mappingK = bgmutil.toBuffer('0x' + key)
  var mappingP = bgmutil.intToBuffer(position)
  mappingP = bgmutil.setLengthLeft(mappingP, 32)
  var mappingKeyBuf = concatTypedArrays(mappingK, mappingP)
  var mappingKeyPreimage = '0x' + mappingKeyBuf.toString('hex')
  var mappingStorageLocation = bgmutil.sha3(mappingKeyPreimage)
  mappingStorageLocation = new bgmutil.BN(mappingStorageLocation, 16)

...
var BgmJSVM = require('bgmchainjs-vm')
var bgmUtil = require('bgmchainjs-util')
var StateManager = require('bgmchainjs-vm/dist/stateManager')
...
class StateManagerCommonStorageDump extends StateManager {
  ...
  putContractStorage (address, key, value, cb) {
    this.keyHashes[bgmUtil.sha3(key).toString('hex')] = bgmUtil.bufferToHex(key)
...
// FIXME: move state manager in BgmJSVM ctr
...
function ExecutionContext () {
  ...
  this.detectNetwork = function (callback) {
    if (this.isVM()) {
      ...
      this.web3().version.getNetwork((err, id) => {
        var name = null
        if (err) name = 'Unknown'
        // https://github.com/bgmchain/EIPs/blob/master/EIPS/eip-155.md
        ...
        if (id === '1') {
          this.web3().bgm.getBlock(0, (error, block) => {
  ...
  this._updateBlockGasLimit = function () {
    if (this.getProvider() !== 'vm') {
      web3.bgm.getBlock('latest', (err, block) => {
        if (!err) {
          // we can't use the blockGasLimit cause the next blocks could have a lower limit : https://github.com/5sWind/bgm/issues/506
...
var transactionDetailsLinks = {
  'Main': 'https://www.bgmchainscan.io/tx/',
  'Rinkeby': 'https://rinkeby.bgmchainscan.io/tx/',
  'Ropsten': 'https://ropsten.bgmchainscan.io/tx/',
  'Kovan': 'https://kovan.bgmchainscan.io/tx/'
}

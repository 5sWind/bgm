...
/**
  ...
  * attention: blocks returned by the event `newBlock` have slightly different json properties whbgmchain web3 or the VM is used
  * trigger 'newBlock'
  *
  */
class TxListener {
  constructor (opt) {
    ...
    opt.event.udapp.register('callExecuted', (error, from, to, data, lookupOnly, txResult) => {
      ...
      var call = {
        ...
        isCall: true,
        returnValue: executionContext.isVM() ? txResult.result.vm.return : bgmJSUtil.toBuffer(txResult.result),
    ...
    opt.event.udapp.register('transactionExecuted', (error, from, to, data, lookupOnly, txResult) => {
      ...
      if (!this._isListening) return // we don't listen
      if (this._loopId && executionContext.getProvider() !== 'vm') return // we seems to already listen on a "web3" network
      executionContext.web3().bgm.getTransaction(txResult.transactionHash, (error, tx) => {
  ...
  _startListenOnNetwork () {
    this._loopId = setInterval(() => {
      ...
      executionContext.web3().bgm.getBlockNumber((error, blockNumber) => {
  ...
  _manageBlock (blockNumber) {
    executionContext.web3().bgm.getBlock(blockNumber, true, (error, result) => {
      if (!error) {
  ...
  _resolveTx (tx, receipt, cb) {
    ...
    if (!tx.to || tx.to === '0x0') { // testrpc returns 0x0 in that case
      ...
      if (!contractName) {
        executionContext.web3().bgm.getCode(tx.to, (error, code) => {
          if (error) return cb(error)
          if (code) {
  ...
  _decodeInputParams (data, abi) {
    data = bgmJSUtil.toBuffer('0x' + data)
    ...
    var abiCoder = new bgmchains.utils.AbiCoder()

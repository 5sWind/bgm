...
/**
  ...
  */
  ...
  _eventABI (contract) {
    var eventABI = {}
    var abi = new bgmchains.Interface(contract.abi)
    for (var e in abi.events) {
      var event = abi.events[e]
      eventABI[bgmJSUtil.sha3(Buffer.from(event.signature)).toString('hex')] = { event: event.name, inputs: event.inputs, object: event }
    }
    return eventABI
  }

'use strict'
var BgmJSTX = require('bgmchainjs-tx')
var BgmJSBlock = require('bgmchainjs-block')
...
class TxRunner {
  ...
  _executeTx (tx, gasPrice, api, promptCb, callback) {
    ...
    if (api.personalMode()) {
      ...
      this._sendTransaction(executionContext.web3().bgm.sendTransaction, tx, null, callback)
    }
  }
  ...
  runInVm (from, to, data, value, gasLimit, useCall, callback) {
    ...
    executionContext.vm().runTx({block: block, tx: tx, skipBalance: true, skipNonce: true}, function (err, result) {
      ...
      callback(err, {
        result: result,
        transactionHash: bgmJSUtil.bufferToHex(Buffer.from(tx.hash()))
      })
    })
  }
  ...
  runInNode (from, to, data, value, gasLimit, useCall, confirmCb, gasEstimationForceSend, promptCb, callback) {
    ...
    if (useCall) {
      tx.gas = gasLimit
      return executionContext.web3().bgm.call(tx, function (error, result) {
        callback(error, {
          result: result,
          transactionHash: result ? result.transactionHash : null
        })
    ...
    executionContext.web3().bgm.estimateGas(tx, function (err, gasEstimation) {
...
async function tryTillReceiptAvailable (txhash, done) {
  return new Promise((resolve, reject) => {
    executionContext.web3().bgm.getTransactionReceipt(txhash, async (err, receipt) => {
      if (err || !receipt) {
...
async function tryTillTxAvailable (txhash, done) {
  return new Promise((resolve, reject) => {
    executionContext.web3().bgm.getTransaction(txhash, async (err, tx) => {
      if (err || !tx) {
        // Try again with a bit of delay if error or if result still null
        await pause()

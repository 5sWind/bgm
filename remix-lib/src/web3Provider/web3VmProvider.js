...
function web3VmProvider () {
  ...
  this.bgm.getTransactionReceipt = function (hash, cb) { return self.getTransactionReceipt(hash, cb) }
...
web3VmProvider.prototype.txWillProcess = function (self, data) {
  ...
  tx.gas = (new bgmutil.BN(util.hexConvert(data.gas).replace('0x', ''), 16)).toString(10)
  if (data.value) {
...
web3VmProvider.prototype.pushTrace = function (self, data) {
  var depth = data.depth + 1 // gbgm starts the depth from 1
...
function getSha3Input (stack, memory) {
  ...
  var memStartDec = (new bgmutil.BN(memoryStart.replace('0x', ''), 16)).toString(10)
  memoryStart = parseInt(memStartDec) * 2
  var memLengthDec = (new bgmutil.BN(memoryLength.replace('0x', ''), 16).toString(10))

...
module.exports = {
  ...
  */
  hexListFromBNs: function (bnList) {
    ...
    for (var k in bnList) {
      var v = bnList[k]
      if (bgmutil.BN.isBN(v)) {
        ret.push('0x' + v.toString('hex', 64))
      } else {
        ret.push('0x' + (new bgmutil.BN(v)).toString('hex', 64)) // TEMP FIX TO REMOVE ONCE https://github.com/bgmchainjs/bgmchainjs-vm/pull/293 is released
      }
    }
    return ret
  },
  ...
  */
  sha3_256: function (value) {
    ...
    var ret = bgmutil.bufferToHex(bgmutil.setLengthLeft(value, 32))
    ret = bgmutil.sha3(ret)
    return bgmutil.bufferToHex(ret)
  },
